...
code_name = 'Electricity_ModelSearch_' #Name for the search 
...
df = pd.read_csv("electricity.csv") #file path & file name
#df = pd.read_csv("https://raw.githubusercontent.com/timothyb0912/pylogit/master/examples/data/electricity_r_data_long.csv") #file path & file name
...
choice_id = df['chid'] 
base_varnames = ['pf','cl','loc','wk','tod','seas'] #all explanatory variables to be included in the model
...
choice_set=[1, 2, 3, 4] #list of alternatives in the choice set
...
model = MixedLogit()
model.fit(X=df[base_varnames], y=choice_var, varnames=base_varnames, alts=alt_var,
          ids=choice_id, avail=av,randvars={'pf':'n','cl':'n','loc':'n','wk':'n','tod':'n','seas':'n'},correlation=True,n_draws=200)
model.summary()
...
#for pf
df['pfl1'] = l1(df['pf'] + (df['pf'] == 0))
df['pfl2'] = ((df['pf']**l2)-1)/l2
df["pfl3"] = ((df['pf']**l3)-1)/l3

#for cl
df['cll1'] = l1(df['cl'] + (df['cl'] == 0))
df['cll2'] = ((df['cl']**l2)-1)/l2
df["cll3"] = ((df['cl']**l3)-1)/l3
...
choice_id = df['chid'] 
ind_id = df['id']
varnames = ['pf','cl','loc','wk','tod','seas','pfl1','pfl2','pfl3','cll1','cll2','cll3'] #all explanatory variables to be included in the model
...
dist = ['n', 'ln', 'tn', 'u', 't', 'f'] #List of random distributions to select from 
...
ps_intercept = None  #(True or False)
...
itr_max = 2500 #150 maximum number of iterations
...
def generate_sol(data,seed,asvars_avail, isvars_avail, rvars_avail, transasvars, bcvars_avail,corvars_avail,asvars_ps, isvars_ps, rvars_ps, 
    ...
    rand_vars = {k:v for k,v in rvars.items() if v!="f"}
...
## Set Random Seed
global_seed = 21052021
np.random.seed(global_seed)
seeds = np.random.choice(24000, 23000, replace = False)
...
def add_new_asfeature(solution,seed):
    ...
    if new_asvar:
        ...
        for i in range(len(solution[0])):
            dis.append(np.random.choice(dist))
        r_vars = dict(zip(solution[0],dis))
