...
code_name = 'Syn_Feb22_' #Name for the search 
...
#file path & file name
df = pd.read_csv("artificial_corr_other_distributions.csv")
df.columns
...
"""
model = MixedLogit()
varnames=['price', 'time', 'conven', 'comfort', 'added_fixed1', 'added_fixed2', 'added_fixed3', 
#           'fixed_nonlinear',
          'meals', 'petfr', 'emipp',
          'added_random1',
          'added_random2', 'added_random3',
          'added_random4', 'added_random5', 
          'rand_nonlinear1',
          'rand_nonlinear2']
X = df[varnames].values
y = df['choice'].values
# df['added_random5'] = -df['added_random5']

# TODO? Change sign of df?
...
model.fit(X, y, ids=df['id'], varnames=varnames, alts=[1,2,3], randvars=randvars,
          n_draws=1000,
          verbose=2,
#           grad=False,
#           hess=False,
#           ftol=1e-2,
           gtol=1e-2,
          correlation=['meals', 'petfr', 'emipp'],
          transvars=['rand_nonlinear1', 'rand_nonlinear2']
         )
model.summary()
"""
...
model.fit(X=df[base_varnames].values, y=choice_var, varnames=base_varnames,isvars=base_isvarnames, alts=alt_var,
          ids=choice_id, avail=av,randvars=rvars,fit_intercept=False,correlation=correl,transvars=bcvars, base_alt = base,gtol=1e-01)
model.summary()
"""
...
##time
df['timel2'] = ((df['time']**l2)-1)/l2
df['timel3'] = ((df['time']**l3)-1)/l3

##coven
df['convenl2'] = ((df['conven']**l2)-1)/l2
df['convenl3'] = ((df['conven']**l3)-1)/l3
"""
...
#           'fixed_nonlinear',
          ...
          'added_random2', 'added_random3',
          'added_random4', 'added_random5', 
          'rand_nonlinear1','rand_nonlinear2', 'nonsig1', 'nonsig2', 'nonsig3'] #all explanatory variables to be included in the model
...
R = 1000 # number of random draws for estimating mixed logit models
Tol = 1e-2 #Tolerance value for the optimazition routine used in maximum likelihood estimation (default value is 1e-06)
...
HMS = 5 #20 #harmony memory size
HMCR_min = 0.6 #0.9 #minimum harmony memory consideration rate
HMCR_max = 0.95 #0.99 #maximum harmony memory consideration rate
PAR_min = 0.2 # 0.8 #min pitch adjustment
PAR_max = 0.8 #0.85 #maximum pitch adjustment
itr_max = 500 #300 #150 maximum number of iterations
v = 0.80 #0.80 #proportion of iterations to improvise harmony. The rest will be for local search
threshold = 0 #15 #threshold to compare new solution with worst solution in memory
...
"""
HMS = int(float(sys.argv[1])) #20 #harmony memory size
HMCR_min = float(sys.argv[2]) #0.9 #minimum harmony memory consideration rate
HMCR_max = float(sys.argv[3]) #0.99 #maximum harmony memory consideration rate
PAR_min = float(sys.argv[4]) # 0.8 #min pitch adjustment
PAR_max = float(sys.argv[5]) #0.85 #maximum pitch adjustment
itr_max = int(float(sys.argv[6])) #300 #150 maximum number of iterations
...
def fit_mnl(dat,as_vars,is_vars,bcvars,choice,alt,id_choice,asc_ind):
    ...
    return(bic,as_vars,is_vars,rand_vars,bc_vars,cor_vars,convergence,pvals,coefs)
...
def fit_mxl(dat,as_vars,is_vars,rand_vars,bcvars,corvars,choice,alt,id_choice,id_val,asc_ind):
    ...
    try:
        ...
        coefs = []
        #pass
    return(bic,as_vars,is_vars,rand_vars,bcvars,corvars,convergence,pvals,coefs)
    ...
    if all_vars:
        ...
        if conv:
            ...
            if all(v for v in sig <= 0.05):
                print("solution at current iteration", sol, convergence)
...
"""
as_vars = ['conven_1', 'conven_2', 'conven_3', 'added_fixed2_1',
  'added_fixed2_2', 'added_fixed2_3', 'added_random2_1', 'added_random2_2',
  'added_random2_3', 'added_random4_1', 'added_random4_2', 'added_random4_3',
  'rand_nonlinear1_1', 'rand_nonlinear1_2', 'rand_nonlinear1_3',
  'time', 'meals', 'emipp', 'added_random3', 'added_random5', 'rand_nonlinear2']
is_vars = []
rand_vars = {'time': 'ln', 'meals': 't', 'emipp': 'u'}
bc_vars=['time', 'meals', 'added_random5']
cor_vars = []
"""
...
hm = HM.copy()
"""
hm = [[13848.828576259515, ['added_fixed1', 'added_fixed2', 'added_fixed3', 'added_random1', 'added_random2', 'added_random3', 'added_random4', 'added_random5', 'comfort', 'conven', 'emipp', 'meals', 'petfr', 'price', 'rand_nonlinear1', 'time'], [], {'added_random1': 't', 'added_random3': 'u', 'added_random4': 'u', 'added_random5': 'ln', 'emipp': 'n', 'meals': 't', 'petfr': 't', 'time': 'u'}, [], [], False], [13854.098985583343, ['added_fixed1', 'added_fixed2', 'added_fixed3', 'added_random1', 'added_random2', 'added_random3', 'added_random4', 'added_random5', 'comfort', 'conven', 'emipp', 'meals', 'petfr', 'price', 'rand_nonlinear1', 'time'], [], {'added_random1': 'n', 'added_random3': 'n', 'added_random4': 'n', 'added_random5': 'n', 'emipp': 'u', 'meals': 'u', 'petfr': 'u', 'rand_nonlinear1': 'n', 'time': 'n'}, [], [], False], [13857.844248490828, ['added_fixed1', 'added_fixed2', 'added_fixed3', 'added_random1', 'added_random2', 'added_random3', 'added_random4', 'added_random5', 'comfort', 'conven', 'emipp', 'meals', 'petfr', 'price', 'rand_nonlinear1', 'time'], [], {'added_random1': 'n', 'added_random3': 'n', 'added_random4': 'n', 'added_random5': 'n', 'emipp': 'u', 'meals': 'u', 'petfr': 'u', 'time': 'n'}, [], [], True], [13878.600997550895, ['added_fixed1', 'added_fixed2', 'added_fixed3', 'added_random1', 'added_random2', 'added_random3', 'added_random4', 'added_random5', 'comfort', 'conven', 'emipp', 'meals', 'petfr', 'price', 'rand_nonlinear1', 'time'], [], {'added_fixed2': 't', 'added_fixed3': 'u', 'added_random1': 'n', 'added_random3': 'n', 'added_random4': 'n', 'added_random5': 'n', 'emipp': 'u'}, [], [], True], [13894.10747970251, ['added_fixed1', 'added_fixed2', 'added_fixed3', 'added_random1', 'added_random2', 'added_random3', 'added_random4', 'added_random5', 'comfort', 'conven', 'emipp', 'meals', 'petfr', 'price', 'rand_nonlinear1', 'time'], [], {'added_fixed1': 't', 'added_fixed2': 'u', 'added_fixed3': 't', 'added_random1': 'u', 'added_random3': 'u', 'added_random4': 'ln', 'added_random5': 'u', 'comfort': 'u', 'conven': 'u', 'emipp': 'n', 'meals': 'ln', 'petfr': 't'}, [], [], True]]
...
def harmony_consideration(har_mem,HMCR_itr,seeds,itr):
    ...
    if  np.random.choice([0,1], p=[1-HMCR_itr,HMCR_itr]) <= HMCR_itr:
        ...
        select_new_asvars_index = np.random.choice([0,1],size = len(har_mem[m_pos][1]), p = [1-HMCR_itr, HMCR_itr])
        select_new_asvars = [i for (i, v) in zip(har_mem[m_pos][1], select_new_asvars_index) if v] 
        ...
        select_new_isvars_index = np.random.choice([0,1],size = len(har_mem[m_pos][2]), p = [1-HMCR_itr, HMCR_itr])
        select_new_isvars = [i for (i, v) in zip(har_mem[m_pos][2], select_new_isvars_index) if v] 
...
def add_new_asfeature(solution,seed):
    ...
    if new_asvar:
        ...
        print("random parameters after pitch adjustment", solution[2])
