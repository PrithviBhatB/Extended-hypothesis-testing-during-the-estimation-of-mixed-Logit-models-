...
from xlogitprit import MixedLogit #for estimation of MNL models
from xlogitprit import MultinomialLogit #for estimation of mixed logit models
...
code_name = 'Berlin_ModelSearch_' #Name for the search 
file_name =  code_name + 'output_' + current_date + '.txt'
...
df = pd.read_csv("berlin_final.csv") #file path & file name
#df['PRICE'] = df['PRICE']*-1
...
choice_id = df['csn'] 
ind_id = df['ID_1']
base_varnames = ['CF', 'RECRE', 'PRICE', 'CF_car', 'CF_stay', 'CF_pt', 'CF_age', 'CF_male', 
                 # 'CF_income', 'CF_child', 'CF_bike', 
                 'BIKELANE', 'BIKESEP', 'DIST6', 'DIST3', 'FREQ_HIGHER', 'FREQ_HIGHEST', 'UNGUARDED', 'GUARDED'] #all explanatory variables to be included in the model
...
base_rvars = {'BIKELANE':'n', 'BIKESEP':'n', 'RECRE':'n', 'DIST6':'n', 'PRICE':'ln', 
'DIST3':'n','FREQ_HIGHER':'n',
'FREQ_HIGHEST':'n','UNGUARDED':'n','GUARDED':'n'}
cor_vars = ['BIKELANE', 'CF', 'BIKESEP', 'PRICE']
choice_set=[1,2,3] #list of alternatives in the choice set
choice_var = df['Choice_'] # the df column name containing the choice variable
alt_var = df['Scenario'] # the df column name containing the alternative variable
base_intercept = False #if intercept needs to be estimated or not (default is False)
...
model.fit(X=df[base_varnames], y=choice_var, varnames=base_varnames, alts=alt_var,
          ids=choice_id, avail=av)
model.summary()
base_model = [model.bic,base_asvarnames,base_isvarnames,{},[],[],base_intercept]
print(base_model)
...
# model = MixedLogit()
# model.fit(X=df[base_varnames], y=choice_var, varnames=base_varnames, alts=alt_var,
#           ids=choice_id,panels=ind_id, avail=av, randvars=base_rvars)
# model.summary()
# #base_model = [model.bic,base_asvarnames,base_isvarnames,{},[],[],base_intercept]
# #print(base_model)
...
##Generic coefficients (remember to name them along with their respective asvarname)
#for time
boxc_l = [] #['L1','L2','L3']
l1 = np.log
l2 =  0.5
...
#for PRICE
#df['PRICE'] = l1(df['PRICE'] + (df['PRICE'] == 0))
df['PRICEL2'] = ((df['PRICE']**l2)-1)/l2
df["PRICEL3"] = ((df['PRICE']**l3)-1)/l3

#for RECRE
df['RECREL1'] = l1(df['RECRE'] + (df['RECRE'] == 0))
df['RECREL2'] = ((df['RECRE']**l2)-1)/l2
df["RECREL3"] = ((df['RECRE']**l3)-1)/l3
...
choice_id = df['csn'] 
ind_id = df['ID_1']
varnames = ['CF', 'RECRE', 'PRICE', 'RECRE',  
                 'BIKELANE', 'BIKESEP', 'DIST6', 'DIST3', 'FREQ_HIGHER', 'FREQ_HIGHEST', 'UNGUARDED', 'GUARDED',
           'CF_car', 'CF_stay', 'CF_pt', 'CF_age', 'CF_male', 'CF_income', 'CF_child', 'CF_bike',] #all explanatory variables to be included in the model
asvarnames = varnames # alternative-specific variables in varnames
isvarnames = [] # individual-specific variables in varnames
choice_set=['1','2','3'] #list of alternatives in the choice set as string
choice_var = df['Choice_'] # the df column name containing the choice variable
alt_var = df['Scenario'] # the df column name containing the alternative variable
av = None  #the df column name containing the alternatives' availability
weight_var = None #the df column name containing the weights 
...
iterations = 2000 #number of iterations for the MLE optimization routine (default value is 2,000)
dist = ['n', 'ln', 'u', 't', 'f'] #List of random distributions to select from 
...
psasvar_ind = [1] + [0] * (len(asvarnames)-1)  #binary indicators representing alternative-specific variables that are prespecified by the user
...
#pspecdist_ind = ["any"] * len(asvarnames) #variables whose coefficient distribution have been prespecified by the modeller
pspecdist_ind = ['f', 'any', 'any','any','any','any','any','any',
 'any', 'any', 'any', 'any', 'f', 'f', 'f', 'f', 'f', 'f', 'f', 'f']

#prespecification on estimation of intercept
ps_intercept = False  #(True or False)

#prespecification on transformations
ps_bctrans = False #(True or False)
...
HMCR_min = 0.9 #minimum harmony memory consideration rate
HMCR_max = 0.99 #maximum harmony memory consideration rate
PAR_min = 0.8 #min pitch adjustment
PAR_max = 0.85 #maximum pitch adjustment
itr_max = 1500 #150 maximum number of iterations
v = 0.80 #proportion of iterations to improvise harmony. The rest will be for local search
...
def df_coeff_col(seed,dataframe,names_asvars,choiceset,var_alt):
    ...
    asvars_new = []
    alt_spec_pos_str = [str(integer) for integer in names_asvars if random_matrix[names_asvars.index(integer)] ==1]
    for i in alt_spec_pos_str:
        for j in choiceset:
            dataframe[i + '_' +'_'.join(j)] = dataframe[i]*([var_alt] == int(j))
            asvars_new.append(i + '_' +'_'.join(j))
...
# generate_sol(df,1,avail_asvars, avail_isvars, avail_rvars, trans_asvars, avail_bcvars,avail_corvars,ps_asvars, ps_isvars, ps_rvars, 
                ...
                while any([v for v in sig if v > 0.05]):
                    ...
                    if nsig_altspec_vars:
                        ...
                        for i in range(len(nsig_altspec_vars)):
                            ...
                            asvars_round2.extend(gen_coeff)
                        rem_asvars = remove_redundant_asvars(asvars_round2,trans_asvars,seed)
                        rem_asvars = sorted(list(set(rem_asvars)))
                    else:
                    ...
                    if ns_cors:
                        ...
                        nscorvars = [k for k,v in most_nscorvars.items() if v >= int(len(randvars.keys())*0.75)]
                    ...
                    if rem_alvars:
                        ...
                        if conv:
                            ...
                            if len(all_ns_int) == len(choice_set)-1:
                                sol[-1] = False
                                return(sol,convergence)
...
## Set Random Seed
global_seed = 11621 #80621 #31052021
...
from math import sin #importing sine function
...
def harmony_consideration(har_mem,HMCR_itr,seeds,itr):
    ...
    if  np.random.rand(1) <= HMCR_itr:
        ...
        select_new_asvars = list(np.random.choice(har_mem[m_pos][1],int((len(har_mem[m_pos][1]))*0.9),replace = False)) #randomly select 90% of the variables from solution at position m_pos in harmony memory
        ...
        select_new_isvars = list(np.random.choice(har_mem[m_pos][2],int((len(har_mem[m_pos][2]))*0.9),replace = False))
...
def pitch_adjustment(sol, har_mem, PAR_itr,seeds,itr):
    ...
    if  np.random.rand(1) <= PAR_itr:
        ...
        improved_harmony = assess_sol(sol,har_mem,seed) 
...
def local_search(improved_harmony,seeds,itr): 
    ...
    if len(best_asvars) == len(asvarnames):
        ...
        improved_harmony = assess_sol(solution_1,improved_harmony,seed)
        ...
        improved_harmony = assess_sol(solution_2,improved_harmony,seed)
        ...
        improved_harmony = assess_sol(solution_3,improved_harmony,seed)
    ...
    best_corvars = [var for var in best_randvars.keys() if var not in best_bcvars]
    solution = [best_asvars, best_isvars, best_randvars, best_bcvars, best_corvars, asc_ind]
    improved_harmony = assess_sol(solution,improved_harmony,seed)
...
def improvise_harmony(HCR_max,HCR_min,PR_max,PR_min,har_mem,max_itr,threshold,itr_prop):
    ...
    while itr < max_itr:
        ...
        HMCR_itr = (HCR_min + ((HCR_max-HCR_min)/max_itr)*itr) * max(0,sin(itr))
        PAR_itr = (PR_min + ((PR_max-PR_min)/max_itr)*itr) * max(0,sin(itr)) 
        #seed = seeds[itr]
        ...
        pa_hm = pitch_adjustment(hmc_sol,har_mem,PAR_itr,seeds,itr)
        ...
        bic_points.append(har_mem[0][0])
        x.append(itr)
        plt.figure()
        plt.xlabel('Iterations')
        plt.ylabel('BIC')
        plt.plot(x,bic_points)
        pp.savefig(plt.gcf())
        ...
        if itr > int(itr_prop*max_itr):
            ...
            #seed = seeds[itr]
            har_mem = local_search(har_mem,seeds,itr)
            ## Sort unique harmony memory from min.BIC to max. BIC
            har_mem = sorted(har_mem, key = lambda x: x[0])
            ## Trim the Harmony memory's size as per the harmony memory size
            har_mem = har_mem[:HMS]
...
new_HM, BICs = improvise_harmony(HMCR_max,HMCR_min,PAR_max,PAR_min,Initial_harmony,itr_max,threshold,v)
