...
code_name = 'SM_Feb22_' #Name for the search 
...
df['GA1_TC'] = df['COST']*((df['GA']==1)&(df['alt']=='TRAIN'))
df['GA0_TC'] = df['COST']*((df['GA']==0)&(df['alt']=='TRAIN'))
...
ind_id =df['ID']
base_varnames = ['TIME', 'GA0_TC', 'COST', 'SEATS','AGE','LUGGAGE']
base_asvarnames = ['TIME', 'COST', 'HEADWAY', 'SEATS'] #['TIME', 'COST', 'HEADWAY','SEATS']
base_isvarnames = ['AGE','LUGGAGE']
...
## Alt-specific variables
#df['GA_PT'] = df['GA']*(df['alt']=='TRAIN')
#df_test['GA_PT'] = df_test['GA']*(df_test['alt']=='TRAIN')

df['AGE_CAR'] = df['AGE']*(df['alt']=='CAR')
#df_test['AGE_CAR'] = df_test['AGE']*(df_test['alt']=='CAR')

df['AGE_SM'] = df['AGE']*(df['alt']=='SM')
#df_test['AGE_SM'] = df_test['AGE']*(df_test['alt']=='SM')

df['AGE_TRAIN'] = df['AGE']*(df['alt']=='TRAIN')
#df_test['AGE_TRAIN'] = df_test['AGE']*(df_test['alt']=='TRAIN')

df['LUGGAGE_TRAIN'] = df['LUGGAGE']*(df['alt']=='TRAIN')
#df_test['LUGGAGE_TRAIN'] = df_test['LUGGAGE']*(df_test['alt']=='TRAIN')

df['LUGGAGE_SM'] = df['LUGGAGE']*(df['alt']=='SM')
#df_test['LUGGAGE_SM'] = df_test['LUGGAGE']*(df_test['alt']=='SM')

df['LUGGAGE_CAR'] = df['LUGGAGE']*(df['alt']=='CAR')
#df_test['LUGGAGE_CAR'] = df_test['LUGGAGE']*(df_test['alt']=='CAR')

df['FIRST_SM'] = df['FIRST']*(df['alt']=='SM')
#df_test['FIRST_SM'] = df_test['FIRST']*(df_test['alt']=='SM')

df['MALE_SM'] = df['MALE']*(df['alt']=='SM')
#df_test['MALE_SM'] = df_test['MALE']*(df_test['alt']=='SM')

df['MALE_CAR'] = df['MALE']*(df['alt']=='CAR')
#df_test['MALE_CAR'] = df_test['MALE']*(df_test['alt']=='CAR')

df['MALE_TRAIN'] = df['MALE']*(df['alt']=='TRAIN')
#df_test['MALE_TRAIN'] = df_test['MALE']*(df_test['alt']=='TRAIN')

df['INCOME_TRAIN'] = df['INCOME']*(df['alt']=='TRAIN')
#df_test['INCOME_TRAIN'] = df_test['INCOME']*(df_test['alt']=='TRAIN')

df['INCOME_CAR'] = df['INCOME']*(df['alt']=='CAR')
#df_test['INCOME_CAR'] = df_test['INCOME']*(df_test['alt']=='CAR')

df['INCOME_SM'] = df['INCOME']*(df['alt']=='SM')
#df_test['INCOME_SM'] = df_test['INCOME']*(df_test['alt']=='SM')

df['WHO_SM'] = df['WHO']*(df['alt']=='SM')
df['WHO_CAR'] = df['WHO']*(df['alt']=='CAR')
df['WHO_TRAIN'] = df['WHO']*(df['alt']=='TRAIN')
...
boxc_l = ['L1','L2'] ##boxc_l is the list of suffixes used to denote manually transformed variables
l1 = np.log
l2 =  0.5
#l3 = 2
...
trans_asvars = ['TIME','COST','HEADWAY']
...
varnames = ['TIME', 'COST', 'HEADWAY', 'SEATS', 
            'GA0_TC', 'AGE_CAR', 'AGE_SM', 'AGE_TRAIN', 'LUGGAGE_TRAIN', 
            'LUGGAGE_SM', 'LUGGAGE_CAR', 'FIRST_SM', 'MALE_SM', 'MALE_CAR', 
            'MALE_TRAIN', 'INCOME_TRAIN', 'INCOME_CAR', 'INCOME_SM', 'TIME_L1','TIME_L2','COST_L1', 'COST_L2','HEADWAY_L1', 'HEADWAY_L2' ] #all explanatory variables to be included in the model

asvarnames = ['TIME', 'COST', 'HEADWAY', 'SEATS', 
            'GA0_TC', 'AGE_CAR', 'AGE_SM', 'AGE_TRAIN', 'LUGGAGE_TRAIN', 
            'LUGGAGE_SM', 'LUGGAGE_CAR', 'FIRST_SM', 'MALE_SM', 'MALE_CAR', 
            'MALE_TRAIN', 'INCOME_TRAIN', 'INCOME_CAR', 'INCOME_SM', 'TIME_L1','TIME_L2','COST_L1', 'COST_L2','HEADWAY_L1', 'HEADWAY_L2' ] # alternative-specific variables in varnames
...
"""
HMS = int(sys.argv[1]) #20 #harmony memory size
...
itr_max = int(sys.argv[6]) #300 #150 maximum number of iterations
v = float(sys.argv[7]) #0.80 #proportion of iterations to improvise harmony. The rest will be for local search
threshold = int(sys.argv[8]) #15 #threshold to compare new solution with worst solution in memory
"""
